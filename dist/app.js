...
class Game {
     ...
     * @returns {boolean} если мы укусили себя, тогда
     * true, иначе false.
     */
    isGameLost() {
        let snakeCopy = this.snake.body.slice();
        let snakeHead = snakeCopy.shift();
        const selfEat = (element) => element.x == snakeHead.x && element.y == snakeHead.y;
        if (snakeCopy.some(selfEat)) {
            clearInterval(this.tickIdentifier);
...
class Snake {
    ...
    /** Метод переносит голову змеи на противоположную сторону
     * игрового поля
     */
    throughTheWall() {
        if (this.body[0].x < 1) {
            this.body[0].x = this.settings.colsCount;
        } else if (this.body[0].x > this.settings.colsCount) {
            this.body[0].x = 1;
        } else if (this.body[0].y < 1) {
            this.body[0].y = this.settings.rowsCount;
        } else if (this.body[0].y > this.settings.rowsCount) {
            this.body[0].y = 1;
        }
    }
}
